...
# Descargar desde: https://raw.githubusercontent.com/problesj/siscda/main/install.sh
...
setup_variables() {
    ...
    INSTALL_DIR="/var/www/html/siscda"
...
clone_repository() {
    print_status "Clonando repositorio SISCDA..."
    
    if [[ -d ".git" ]]; then
        print_warning "El directorio ya es un repositorio Git"
        print_status "Actualizando desde el repositorio remoto..."
...
configure_application() {
    ...
    if [[ -f "config.example.php" ]]; then
        ...
        print_success "Archivo de configuración copiado"
    else
        print_warning "Archivo config.example.php no encontrado"
    fi
    
    # Copiar archivo .htaccess de ejemplo
    if [[ -f ".htaccess.example" ]]; then
        cp .htaccess.example .htaccess
        print_success "Archivo .htaccess copiado"
    else
